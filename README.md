...
AI/LLM エンジニア向けポートフォリオ提出用の RAG システムです。Phase1 は、文書アップロードから検索、回答生成、citation、評価、監査ログまでを Docker Compose で再現できる構成を目標にします。
...
PR-02 時点で、ローカル検証用の Docker/Compose 土台を追加しています。

- `frontend`: Vite dev server。ブラウザから `http://localhost:5173` でアクセスします。
- `backend`: FastAPI。`/health` は liveness、`/ready` は DB 接続を含む readiness です。PR-02時点の `/ready` は DB-only で、Qdrant / LLM を含む RAG readiness は後続PRで追加します。
...
`docker-compose.ci.yml` は CI smoke 専用のスタンドアロン構成です。Ollama を起動せず、fake adapter 前提で Docker build、migration、seed、backend readiness、frontend build artifact、worker 起動を確認する入口にしています。worker の実ジョブ投入・完了確認は Worker / Job PR で追加します。
