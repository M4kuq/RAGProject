...
| `AWS_DEPLOY_ROLE_ARN` | 手動workflow実行時のfallback。`terraform output -raw github_deploy_role_arn` |
...
`aws-demo.ps1 up` はfresh apply直後の `github_deploy_role_arn` を検証済み `deployment_config` としてapp/frontend workflowへ直接渡します。そのため初回作成時に、まだ存在しないrole ARNを先回りしてrepository variableへ設定する必要はありません。
