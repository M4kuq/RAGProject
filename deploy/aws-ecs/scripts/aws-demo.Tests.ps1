...
Assert-True ($content -match '\$script:ExpectedBranch\s*=\s*"main"') "the main branch guard is required"
...
Assert-True ($lifecycleWorkflow -match 'TF_VAR_github_deploy_branch:\s+main') "lifecycle Terraform branch must be main"
Assert-True ($lifecycleWorkflow -match 'refs/heads/main') "lifecycle workflow must retain the main branch guard"
Assert-True ($lifecycleWorkflow -match 'ref:\s+main') "lifecycle workflow must checkout main"
...
Assert-True ($appWorkflow -match 'group: aws-demo-runtime-main') "app deploy must share the runtime teardown lock"
Assert-True ($frontendWorkflow -match 'group: aws-demo-runtime-main') "frontend deploy must share the runtime teardown lock"
Assert-True ($lifecycleWorkflow -match 'aws-demo-up-orchestrator-main') "up orchestration must avoid deadlocking dispatched deploy workflows"
Assert-True ($appWorkflow -match 'refs/heads/main' -and $appWorkflow -match 'ref:\s+main') "app deploy must guard and checkout main"
Assert-True ($frontendWorkflow -match 'refs/heads/main' -and $frontendWorkflow -match 'ref:\s+main') "frontend deploy must guard and checkout main"
...
Assert-True ($appWorkflow -match 'git merge-base --is-ancestor "\$SOURCE_SHA" "origin/main"') "app workflow must restrict source_sha to the main branch history"
Assert-True ($frontendWorkflow -match 'git merge-base --is-ancestor "\$SOURCE_SHA" "origin/main"') "frontend workflow must restrict source_sha to the main branch history"
...
Assert-True ($planWorkflow -match "github\.ref == 'refs/heads/main'") "manual Terraform plan must be restricted to main"
