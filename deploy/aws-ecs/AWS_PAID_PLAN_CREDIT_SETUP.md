...
- Terraform state、lock、GitHub OIDCと外部入力用Secrets Manager containerはbootstrapとして保持する。
...
## 4. CloudFront default domain

このdemoは独自ドメイン、Route 53 hosted zone、ACM certificateを必須にしない。viewer HTTPSはCloudFront default domainで提供し、API originはCloudFront VPC Originからprivate subnetのinternal ALBへ接続する。ALBはインターネットから直接到達できず、private側はHTTP 80とする。

独自viewer domainが後で必要になった場合だけ、`us-east-1`のACM certificateとCloudFront aliasを別の変更として追加する。Route 53のドメイン登録・移管・更新料にはAWS promotional creditを使えず、登録カードへ請求されるため、このdemo検証では追加しない。
...
- runtime planがOIDC、state、lockを削除しない。
- VPC Originがinternal ALB、private subnet、HTTP 80を使い、`apne1-az3`を除外している。
...
- production前にFargate taskのprivate subnet化、VPC endpoints、least privilege、監査ログ、backupを再設計する。
...
- Account ID、resource ID、ARN、bucket名
- 支払情報
