...
`agentic_router` 縺ｮ planner 縺ｯ `ROUTER_LLM_PLANNER_MODEL_NAME` 縺ｧ回答生成モデルと別に指定できます。LM Studio で 9B が重い場合でも、回答生成は Qwen3.5 9B、検索戦略 planner は Qwen3.5 4B に分けられます。未設定時は `GENERATION_MODEL_NAME` を使います。
