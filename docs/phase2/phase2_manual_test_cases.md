...
| P2-TC-1605 | LangGraph Agentic RAG | Chat UI LangGraph Agentic | Select `LangGraph Agentic` in Chat and send a safe synthetic question. | `/rag/ask` sends `strategy=langgraph_agentic`; answer uses citations/confidence or returns `422 no_context_found` safely. | Same retrieval-only tool boundary as Auto. |
| P2-TC-1606 | LangGraph Agentic RAG | Safe trace | Inspect Retrieval Debug for a `langgraph_agentic` run. | Trace shows provider, tool counts, finalize/budget flags, and LangGraph latency fields only. | No raw prompt, full context, raw chunk text, token, or secret. |
