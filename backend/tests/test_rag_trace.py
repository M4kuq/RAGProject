...
def test_latency_tracker_excludes_nested_langchain_agentic_spans_from_retrieval_total() -> None:
    clock = _Clock([0.0, 0.2])
    tracker = LatencyTracker(clock=clock)
    tracker.record_ms("langchain_agentic_ms", 100)
    tracker.record_ms("langchain_planning_ms", 25)
    tracker.record_ms("langchain_tool_execution_ms", 70)
    tracker.record_ms("query_embedding_ms", 10)
    tracker.record_ms("qdrant_search_ms", 20)
    tracker.record_ms("rdb_final_check_ms", 5)
    tracker.record_ms("retrieval_items_persist_ms", 7)
