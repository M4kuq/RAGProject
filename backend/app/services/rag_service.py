Warning: truncated output (original token count: 47559)
Total output lines: 4915
...
class RagService:
            ...
            saved_items = self.repository.save_items(
                ...
                items=[
                    _hybrid_run_item_input(
                        candidate,
                        rerank_result=rerank_by_chunk_id.get(candidate.chunk.document_chunk_id),
                        final_rank=index,
                        selected_flag=index <= selected_count,
                        fusion_method=fusion_method,
                    )
    ...
    def _persist_agentic_result(
        ...
        *,
        ...
        selected_count = (
            0 …7559 tokens truncated…alStrategy) -> None:
...
def _hybrid_run_item_input(
    ...
    *,
    ...
    return RetrievalRunItemInput(
        ...
        retrieval_score=_decimal_score(candidate.retrieval_score),
        rerank_score=(
            _decimal_score(rerank_result.rerank_score) if rerank_result is not None else None
        ),
        rank_order=candidate.rank_order,
