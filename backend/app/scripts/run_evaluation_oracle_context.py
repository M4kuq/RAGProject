...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=("Compare a frozen local_accuracy_dev_v1 run with dev-only Oracle Context.")
    )
    ...
    if settings.app_env == "production" or not args.confirm_local_only:
        raise SystemExit("oracle_context_local_confirmation_required")

    try:
        replay_payload = json.loads(args.r_judge_replay.read_text(encoding="utf-8"))
        if not isinstance(replay_payload, dict):
            raise ValueError
    except (OSError, UnicodeError, ValueError, json.JSONDecodeError):
        print(
            json.dumps(
                {"status": "blocked", "reason_code": "oracle_r_judge_replay_unreadable"},
                sort_keys=True,
            )
        )
        return 2
